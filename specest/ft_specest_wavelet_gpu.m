function [spectrum,freqoi,timeoi] = ft_specest_wavelet_gpu(dat, time, varargin)
...
for ifreqoi = 1:nfreqoi
  ...
  wltspctrm{ifreqoi} = gpuArray(complex(zeros(1,endnsample)));
  wltspctrm{ifreqoi} = fft(gpuArray(wavelet),[],1)';
...
%     fprintf([str, '\n']);
  ...
  if ~isempty(reqtimeboi)
    ...
    spectrum(:,ifreqoi,reqtimeboiind) = gather(dum(:,reqtimeboi));
  end
end
